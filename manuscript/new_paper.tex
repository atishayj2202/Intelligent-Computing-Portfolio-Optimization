...
This paper presents \textbf{AOBL-SOS} (Adaptive Opposition-Based Learning Symbiotic Organisms Search), a diversity-aware and feasibility-preserving optimization framework for constructing a non-convex portfolio. Empirical validation is performed on a point-in-time equity universe using an expanding walk-forward testing horizon covering 2018--2024, represented by seven out-of-sample testing windows and eliminating survivorship bias. When executed with large-scale $AUM = \$100\text{M}$ Almgren-Chriss friction for each rebalancing, AOBL-SOS significantly outperforms the baseline metaheuristics in terms of out-of-sample risk-adjusted performance: OS Sharpe ratio 0.841, CAGR 17.27\%, versus OS Sharpe 0.733 for SOS, 0.734 for PSO, 0.727 for GA, and 0.609 for DE. A six-variant walk-forward study also shows that incorporating drawdown penalization with centroid-based adaptive opposition yields superior risk-adjusted allocation. Moreover, the out-of-sample Sharpe ratio exceeds the trial-inflation hurdles, while both inter-grid selection variability and a CSCV Probability of Backtest Overfitting $\text{PBO} = 0.6998$ ($69.98\%$ confidence) are confirmed. Carhart 4-factor regressions demonstrate that performance is driven by systematic market beta ($\beta_{\text{MKT}} = 0.924, t=56.94$), value ($\beta_{\text{HML}} = 0.291, t=10.23$), and momentum factor exposure ($\beta_{\text{MOM}} = 0.638, t=19.96$).
\end{abstract}
...
Modern quantitative asset allocation is an exercise in navigating highly constrained non-convex mathematical spaces. Portfolio selection is based on Markowitz's \cite{markowitz1952} mean-variance framework. Two fundamental bottlenecks limit the use of portfolio selection in practice by institutions: (1) extreme sensitivity to input covariance and return estimates \cite{olmo2023} and (2) the requirement of continuous, twice-differentiable constraint sets \cite{li2024cardinality}. In some cases, institutional mandates place strict bounds on operations such as exact cardinality constraints ($\|\mathbf{w}\|_0 = K = 30$), which limit transaction overhead, or position exposure caps ($w_i \le 20\%$), which guarantee diversification. This renders the optimization problem an NP-hard Mixed-Integer Nonlinear Programming (MINLP) problem \cite{li2024cardinality}.

Typical commercial solvers address cardinality bounds using convex relaxation or greedy rounding heuristics, which often produce suboptimal weights that violate operational requirements or fail in out-of-sample market stress testing \cite{bertsimas1999, fabozzi2007}. By contrast, simple diversification strategies such as the equal-weighting $1/N$ approach often outperform advanced optimization algorithms because of estimation error in the sample covariance matrix \cite{demiguel2009}. Metaheuristic techniques based on population search, such as Genetic Algorithms (GA) \cite{holland1992}, Particle Swarm Optimization (PSO) \cite{kennedy1995}, and Differential Evolution (DE) \cite{storn1997}, show strong promise for navigating non-convex financial landscapes \cite{zheng2023}. However, conventional metaheuristics have serious drawbacks in high-dimensional financial problems because candidate solutions often converge prematurely in the early iterations of the search process \cite{bailey2024}.

Four main contributions overcome both the premature convergence and the handling of constraints in this study:

\begin{enumerate}

\item \textbf{Feasibility-Preserving Simplex Encoding:} The construction of a continuous latent solution encoding $\mathbf{x} \in \mathbb{R}^n$ is coupled with a deterministic mapping of the active subset $(\mathbf{S}(\mathbf{x}), \boldsymbol{\theta}(\mathbf{x}))$ to constrain the unrestricted optimization parameters to the capped probability simplex, respecting cardinality and forming feasible individuals in the population. We forgo repair mechanisms.
 \item \textbf{Centroid-Based Adaptive Opposition Mechanism:} Departing from uniform permutation opposition \cite{tizhoosh2005}, we leverage an opposition operator that acts on the mean of stagnant allocations in population $\mathbf{c}$, $\mathbf{x}_{\text{opp}} = \mathbf{c} + \gamma(\mathbf{c} - \mathbf{x})$. The factor $\gamma$ increases in real time in response to population diversity decay.
 \item \textbf{Survivorship-Bias-Free Walk-Forward Protocol:} We apply temporal filtering of the investable universe at each decision point, based solely on historical data availability, using seven distinct walk-forward windows over Jan 1, 2018--Dec 31, 2024. Each step begins on the last decision date and uses only data known prior to that point, removing survivorship and out-of-sample testing bias.
 \item \textbf{Institutional Execution \& Overfitting Validation:} We explore the robustness of our model under typical market trading environments, including the Almgren-Chriss square-root model of market impact \cite{horst2024}. Performance is also validated against standard overfitting metrics, including Combinatorial Symmetric Cross-Validation (CSCV) \cite{bailey2017}, Carhart 4-factor regressions, varying asset universe size ($10\text{M}-5\text{B}$), and VIX market regime conditions.
...
Theoretical alpha frequently vanishes in live execution due to bid-ask spread friction and market impact shortfall \cite{horst2024}. Institutional optimal execution frameworks, such as the Almgren-Chriss square-root model \cite{horst2024}, quantify the non-linear execution costs incurred when trading block orders relative to Average Daily Volume (ADV). Furthermore, high-dimensional strategy selection creates extreme selection bias. Bailey and López de Prado (2014) \cite{bailey2024} establish that backtest overfitting can be quantified via Combinatorial Symmetric Cross-Validation (CSCV) and the Deflated Sharpe Ratio (DSR), which adjust out-of-sample performance for the number of candidate configurations explored \cite{olmo2023, prado2018}.
...
Out-of-sample evaluation is based on the strict enforcement of the non-linear Almgren-Chriss square-root execution shortfall model \cite{horst2024}. When rebalancing portfolio value $V_t$ from weights $\mathbf{w}_{t-1}$ to $\mathbf{w}_t$, trading volume for asset $i$ is $Q_i = V_t \cdot |w_{i, t} - w_{i, t-1}|$. The $\Delta P_i / P_i$ fractional market impact cost is calculated dynamically:
\begin{equation}
