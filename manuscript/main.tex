...
To synthesize the multi-dimensional evaluations conducted throughout this study, we rank the candidate strategies across three core institutional pillars: (1) \textbf{Capital Preservation} (minimizing CVaR and maximizing Calmar), (2) \textbf{Execution Viability} (decay in Sharpe from frictionless to illiquid), and (3) \textbf{Crisis Resilience} (net Sharpe during the 2020 COVID Crash and 2022 Drawdown). 

\begin{enumerate}
    \item \textbf{AOBL-SOS (Rank \#1)}: Unquestionably superior in \textit{Capital Preservation} (highest Calmar, lowest kurtosis) and \textit{Crisis Resilience} (crushing all benchmarks during the 2020 and 2022 shocks). Its rigid constraints also make it the undisputed leader in \textit{Execution Viability}, suffering the lowest slippage decay.
    \item \textbf{SOS Baseline (Rank \#2)}: A strong runner-up. It provides decent execution viability and avoids the tail-risks of Markowitz, but lacks the adaptive stagnation-breaking power to reach optimal allocations, consistently trailing AOBL-SOS in Calmar ratio.
    \item \textbf{Markowitz Max Sharpe (Rank \#3)}: Performs exceptionally well in pure, unconstrained bull markets (e.g., 2021 Post-COVID rally), but fails spectacularly during severe drawdowns. It is extremely fragile to execution costs due to high turnover.
    \item \textbf{Min Variance \& Equal-Weight (Rank \#4)}: These heuristic baselines occasionally perform well in isolated regimes but fail to generate consistent alpha across dynamic, multi-year cycles.
\end{enumerate}
