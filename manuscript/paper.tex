...
This paper introduces \textbf{AOBL-SOS} (Adaptive Opposition-Based Learning Symbiotic Organisms Search), a diversity-aware, feasibility-preserving optimization framework designed for non-convex portfolio construction. To eliminate survivorship bias, empirical validation is executed over a point-in-time equity universe across an expanding walk-forward horizon (2018--2024) comprising seven distinct out-of-sample testing windows. Under standard 10 bps rebalancing friction, AOBL-SOS achieves superior out-of-sample risk-adjusted performance (chained Sharpe ratio 0.841, CAGR 17.27\%), adjusting to Sharpe 0.827 under large-scale $AUM = \$100\text{M}$ Almgren-Chriss market impact execution, relative to baseline metaheuristics (SOS Sharpe 0.733, PSO Sharpe 0.734, GA Sharpe 0.727, DE Sharpe 0.609), shrinkage estimators (Ledoit-Wolf Sharpe 0.699), and the 1/N equal-weight benchmark (Sharpe 0.676). A 6-variant walk-forward ablation study confirms that combining path-dependent drawdown penalization with centroid-based adaptive opposition improves risk-adjusted allocation. Furthermore, multiple-testing validation yields a Deflated Sharpe Ratio of $\text{DSR} = 0.9522$ ($95.22\%$ confidence), a Probabilistic Sharpe Ratio of $\text{PSR} = 1.0000$ ($100\%$ confidence), and an Implied Hurdle Sharpe of $SR_0 = 0.1951$, confirming that realized out-of-sample Sharpe ratio exceeds trial-inflation hurdles, alongside a CSCV Probability of Backtest Overfitting of $\text{PBO} = 0.6998$ reflecting intra-grid selection variability. Carhart 4-factor multi-factor regressions demonstrate that performance is driven by systematic market beta ($\beta_{\text{MKT}} = 0.924, t=56.94$), value ($\beta_{\text{HML}} = 0.291, t=10.23$), and momentum factor exposure ($\beta_{\text{MOM}} = 0.638, t=19.96$).
\end{abstract}
...
Modern quantitative asset allocation requires navigating highly constrained, non-convex mathematical spaces. While Markowitz's \cite{markowitz1952} mean-variance framework provides the foundational framework for portfolio selection, its practical deployment in institutional settings is obstructed by two fundamental bottlenecks: (1) extreme sensitivity to input covariance and return estimates \cite{michaud1989}, and (2) the requirement for continuous, twice-differentiable constraint sets \cite{black1992}. When institutional mandates demand strict operational bounds—specifically exact cardinality constraints ($\|\mathbf{w}\|_0 = K = 30$) to restrict transaction overhead and position exposure caps ($w_i \le 20\%$) to enforce diversification—the optimization problem transforms into an NP-hard Mixed-Integer Nonlinear Programming (MINLP) challenge \cite{chang2000}.

Standard commercial solvers address cardinality bounds via convex relaxations or greedy rounding heuristics, which frequently yield suboptimal weights that violate operational mandates or collapse under out-of-sample market stress \cite{bertsimas1999, fabozzi2007}. Conversely, naive diversification models such as the equally-weighted $1/N$ portfolio frequently outperform complex analytical optimizers due to the estimation error embedded in sample covariance matrices \cite{demiguel2009}. Population-based metaheuristics—such as Genetic Algorithms (GA) \cite{holland1992}, Particle Swarm Optimization (PSO) \cite{kennedy1995}, and Differential Evolution (DE) \cite{storn1997}—demonstrate substantial potential for traversing non-convex financial landscapes \cite{gilli2001, cheng2014}. However, standard metaheuristics suffer from premature convergence when applied to high-dimensional financial problems. Candidate allocations rapidly lose population diversity in early search iterations, trapping solutions in fragile local minima that appear optimal in-sample but degrade severely out-of-sample \cite{bailey2014}.

This study addresses the premature convergence and constraint handling bottlenecks through four primary contributions:
\begin{enumerate}
    \item \textbf{Feasibility-Preserving Simplex Encoding:} We design a continuous latent solution encoding $\mathbf{x} \in \mathbb{R}^n$ with deterministic active subset mapping $(\mathbf{S}(\mathbf{x}), \boldsymbol{\theta}(\mathbf{x}))$ that maps unconstrained optimization variables directly onto the cardinality-constrained capped probability simplex. Every generated solution in the population is intrinsically feasible, eliminating post-hoc repair heuristics.
    \item \textbf{Centroid-Based Adaptive Opposition Mechanism:} Rather than applying static permutation opposition \cite{tizhoosh2005}, we introduce a centroid-based opposition operator $\mathbf{x}_{\text{opp}} = \mathbf{c} + \gamma(\mathbf{c} - \mathbf{x})$ that reflects stagnated allocations across the population centroid $\mathbf{c}$, where $\gamma$ adapts dynamically to real-time population diversity collapse.
...
To evaluate risk exposures, we regress daily out-of-sample portfolio excess returns $R_{p, t} - R_{f, t}$ against contemporaneous daily Carhart 4-factor returns (MKT, SMB, HML, MOM) with Newey-West / HAC robust standard errors ($maxlags=5$):
\begin{equation}
R_{p, t} - R_{f, t} = \alpha + \beta_{\text{MKT}} \text{MKT}_t + \beta_{\text{SMB}} \text{SMB}_t + \beta_{\text{HML}} \text{HML}_t + \beta_{\text{MOM}} \text{MOM}_t + \epsilon_t
\end{equation}
...
\begin{tabular}{lcccc}
\toprule
\textbf{Market Volatility Regime} & \textbf{Observations} & \textbf{\% of Sample} & \textbf{Net Sharpe} & \textbf{Net Ann. Return (CAGR)} \\
\midrule
...
Across $M=20$ independent random execution seeds, AOBL-SOS achieves a Weight Stability index $WS = 1 - \frac{1}{M} \sum_{m=1}^M \|\mathbf{w}^{(m)} - \bar{\mathbf{w}}\|_1 = 0.9567$ and a mean pairwise Jaccard overlap of $J = 0.2689$ across active selection sets $\mathbf{S}$. The high $WS$ ($95.67\%$) confirms that position exposure magnitudes remain stable across executions, while the Jaccard overlap ($26.89\%$) reflects healthy rotational variation among substitute constituents. Top consistently selected assets include Northrop Grumman (NOC), O'Reilly Automotive (ORLY), Regeneron Pharmaceuticals (REGN), Tesla (TSLA), and Netflix (NFLX).

\subsection{Convergence \& Diversity Dynamics}
Figure \ref{fig:diversity} (left) illustrates objective loss convergence $-g(\mathbf{w}) = \text{SR} - \lambda |\text{MDD}|$ (plotted as negative loss where higher values represent superior risk-adjusted performance) and population diversity $D_t$ over 150 iterations. While standard SOS exhibits rapid diversity collapse during search, AOBL-SOS triggers adaptive centroid opposition upon stagnation, periodically restoring population diversity and delaying diversity collapse relative to standard SOS.
...
This study introduced AOBL-SOS, a diversity-aware, feasibility-preserving metaheuristic optimization framework for cardinality-constrained portfolio construction. By combining bounded simplex solution encodings, adaptive centroid-based opposition, and path-dependent drawdown penalization, the model overcomes premature convergence in high-dimensional financial search spaces. Evaluated over a point-in-time equity universe across an expanding walk-forward horizon (2018--2024), AOBL-SOS achieves a net out-of-sample Sharpe ratio of 0.841 under standard 10 bps rebalancing friction (0.827 under $AUM = \$100\text{M}$ Almgren-Chriss execution) and 17.27\% annualized CAGR return. Multiple-testing statistical validation yields a Deflated Sharpe Ratio of $\text{DSR} = 0.9522$ and an implied hurdle Sharpe of $SR_0 = 0.1951$, confirming that realized out-of-sample Sharpe exceeds selection-adjusted hurdles, while a CSCV Probability of Backtest Overfitting of $\text{PBO} = 0.6998$ highlights configuration-selection instability across hyperparameter variations.
